...
export default function SeoTable() {
  ...
  // Track which row's slug was just copied (by id)
  ...
  const fetchSeoPages = useCallback(async (
    ...
    try {
      ...
      console.debug('[SeoTable] Raw API response:', response);
      ...
      if (pagination && Array.isArray(pagination.data)) {
        ...
        console.warn('[SeoTable] Unexpected pagination structure', pagination);
        setSeoPages([]);
        setFilteredSeoPages([]);
      }
    } catch (error) {
  ...
  const handleDelete = async (seopage: Page) => {
    ...
    const token = getCookie('auth_token') || '';
    try {
  const resp = await deleteSeoPage(token, seopage.id, { page_type: 1 });
      if (resp) {
        // Remove locally
  ...
  // Initial load only with guard to prevent double fetch in React 18 Strict Mode
                ...
                {filteredSeoPages.map((seopage) => (
                  <TableRow key={seopage.id}>
                    ...
                    <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                      <ThreeDotDropdown
                        options={[
                          { label: 'Edit', onClick: () => { window.location.href = `/seo_pages/edit/${seopage.id}` } },
